...
9. **景表法チェックを執筆時に行う。** 数値実績は出典フル記載必須。給付金・価格は条件併記必須。比較・最上級語は根拠なしで使用禁止。詳細はrules/feedback_legal.md参照。
10. **文体・構成FBを毎回適用する。** 語尾統一・strong締め・段落分け・テーブル重複回避等の詳細はrules/feedback_writing.md、rules/feedback_structure.md参照。
